...
class Dual7dof:
    def __init__(self, ref, config, param, previous_data) -> None:
        ...
        ay     = ay_n * self.a_s
        # formulate array
        u = ca.vertcat(ddelta_n, dT_n, ax_n, ay_n)
        self.u_s = np.array([self.ddelta_s, self.dT_s, self.a_s, self.a_s])
        ...
        """ Constraint term """
        # constrain ax and ay
        g_1 = ax - (dvx_t - dpsi * vy)
        g_2 = ay - (dvy_t + dpsi * vx)
        g = ca.vertcat(g_1, g_2)
